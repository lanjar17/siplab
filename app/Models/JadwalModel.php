...
class JadwalModel extends Model
{
    ...
    public function cekTersedia($id_ruangan)
    {
        ...
        $builder->join('ruangans', 'peminjaman.id_ruangan = ruangans.id_ruangan');
        $builder->where('jadwal.status_jadwal', 1);
        $builder->where('ruangans.id_ruangan', $id_ruangan);
    ...
    {
        return $this->query("SELECT * FROM jadwal INNER JOIN peminjaman, ruangans WHERE jadwal.id_peminjaman=peminjaman.id_peminjaman AND peminjaman.id_ruangan=ruangans.id_ruangan AND jadwal.status_jadwal=2 AND ruangans.id_ruangan='")->getResult();
    }
